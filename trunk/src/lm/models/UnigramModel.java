...
public class UnigramModel {
	...
	public UnigramModel(String[] vocabulary, boolean handleUnknownWord) {
		...
		if (handleUnknownWord == false)populateModel(vocabulary);
		else populateModelHandlingUnknownWord(vocabulary);
	}
...
private void populateModelHandlingUnknownWord(String[] vocabulary){
	...
	public Unigram generateNextWord() {
		...
		for (Unigram word : words) {
			...
			double wordProb = wordCount.divide(nrOfTokens, 20, BigDecimal.ROUND_HALF_UP);
	...
	public String toString() {
		...
		for (String unigram : unigramModel.keySet()) {
			sb.append(unigram + " " + unigramModel.get(unigram).getCount()+ " " + unigramModel.get(unigram).getProbability());
			sb.append("\n");
		}
